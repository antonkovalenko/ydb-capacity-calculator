...
// Please use CapacityCalculator class from CapacityCalculator.js instead
// ============================================================================
//
// This file is kept for backward compatibility only.
// All functions now delegate to the new CapacityCalculator class.
//
// Migration guide:
// Old: const result = performCalculations(serverConfig, requirements);
// New: const calc = new CapacityCalculator();
//      const result = calc.calculateServersNeeded(serverConfig, requirements);
//
// ============================================================================

// Import CapacityCalculator for internal use
let CapacityCalculator;
if (typeof require !== 'undefined') {
    CapacityCalculator = require('./CapacityCalculator');
} else if (typeof window !== 'undefined' && window.CapacityCalculator) {
    CapacityCalculator = window.CapacityCalculator;
}

// Create a singleton instance for backward compatibility
const calculator = CapacityCalculator ? new CapacityCalculator() : null;

/**
 * @deprecated Use CapacityCalculator.calculateServersNeeded() instead
 */
function getDominantResource(serversByResource) {
    if (!calculator) {
        // Fallback to original implementation if CapacityCalculator not available
    ...
    return calculator._getDominantResource(serversByResource);
}
...
function calculateStorageServers(serverConfig, capacityRequirements) {
    ...
    return calculator._calculateStorageServers(serverConfig, capacityRequirements);
}
...
function calculateCoresServers(serverConfig, capacityRequirements) {
    ...
    return calculator._calculateCoresServers(serverConfig, capacityRequirements);
}
...
function calculateRamServers(serverConfig, capacityRequirements) {
    ...
    return calculator._calculateRamServers(serverConfig, capacityRequirements);
}
...
function performCalculations(serverConfig, capacityRequirements) {
    ...
    // Normalize serverConfig to ensure all required fields are present
    // Use sensible defaults for backward compatibility with old tests
    const normalizedConfig = {
        coresPerServer: serverConfig.coresPerServer || 32,
        ramPerServer: serverConfig.ramPerServer || 128,
        nvmeDevicesPerServer: serverConfig.nvmeDevicesPerServer || 0,
        hddDevicesPerServer: serverConfig.hddDevicesPerServer || 0,
        vdisksPerHddPdisk: serverConfig.vdisksPerHddPdisk || 8,
        vdisksPerNvmePdisk: serverConfig.vdisksPerNvmePdisk || 16,
        ...serverConfig
    };
    
    const result = calculator.calculateServersNeeded(normalizedConfig, capacityRequirements);
    return {
        serversByResource: result.serversByResource,
        dominantResource: result.dominantResource,
        finalServers: result.finalServers
    };
}

/**
 * @deprecated Use CapacityCalculator.calculateProvidedCapacity() instead
 */
function calculateProvidedCapacity(serverConfig, serverCount) {
    // For backward compatibility, allow server counts < 9 by using fallback implementation
    if (!calculator || serverCount < 9) {
        // Fallback to original implementation
        const hddVdisksPerServer = (serverConfig.hddDevicesPerServer || 0) * (serverConfig.vdisksPerHddPdisk || 0);
        const nvmeVdisksPerServer = (serverConfig.nvmeDevicesPerServer || 0) * (serverConfig.vdisksPerNvmePdisk || 0);
    ...
    return calculator.calculateProvidedCapacity(normalizedConfig, serverCount);
}
